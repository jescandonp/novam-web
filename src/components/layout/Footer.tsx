    ...
    <footer className="bg-nova-navy text-white" aria-label="Pie de página">
      ...
      <div className="container mx-auto max-w-[1280px] px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
          ...
          {/* Columna 4: Ingenieros por zona */}
          <div>
            <h3 className="text-sm font-display font-bold text-white uppercase tracking-widest mb-5">
              Ingenieros de zona
            </h3>
            <address className="not-italic">
              <ul className="space-y-3.5" aria-label="Contactos por zona">
                {ZONE_CONTACTS.map(({ zone, city, name, phoneHref, phone }) => (
                  <li key={`${zone}-${name}`}>
                    <a
                      href={phoneHref}
                      className="group flex flex-col gap-0.5"
                      aria-label={`${name} — ${zone} — ${phone}`}
                    >
                      <span className="text-[10px] font-medium uppercase tracking-widest text-nova-cyan/70 font-sans">
                        {zone} · {city}
                      </span>
                      <span className="text-sm text-white/80 font-sans group-hover:text-nova-cyan transition-colors">
                        {name}
                      </span>
                      <span className="flex items-center gap-1.5 text-xs text-white/45 font-sans group-hover:text-nova-cyan/70 transition-colors">
            ...
            {/* Email general + LinkedIn */}
            <div className="mt-5 pt-4 border-t border-white/10 space-y-3">
              <a
                href={`mailto:${contactInfo.email}`}
                className="flex items-center gap-2 text-sm text-white/60 hover:text-nova-cyan transition-colors font-sans"
              >
